...
namespace Bidvalet.Droid.SortView
{
...
//					anim.Start ();
					...
					// Swap
					var from = GetPositionForView (mobileView);
					var to = GetPositionForView (switchView);
					if(from != to ) {
						((IDraggableListAdapter)Adapter).SwapItems (from, to);
					}
...
//							if(from != to )//|| from < 0|| from > this.Count || to < 0 || to >= this.Count)
//							//return;
//								((IDraggableListAdapter)Adapter).SwapItems (from, to);
//						}catch(Exception ex){
//						Log.Debug("DRGLV",ex.Message);
//							Log.Debug("",ex.StackTrace);
//							//int f = SelectedItemPosition>0?SelectedItemPosition:0;
//							//int t = GetPositionForView (SelectedView);
//							//return;
//							//((IDraggableListAdapter)Adapter).SwapItems (0, 1);
//						}
						//((IDraggableListAdapter)Adapter).SwapItems (from, to);
//					};
				}
		...
		void TouchEventsEnded ()
		{
			...
			if (mCellIsMobile) {
				...
				adapter.mMobileCellPosition = int.MinValue;
