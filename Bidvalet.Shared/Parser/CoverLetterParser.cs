				...
				try
				{
					while ((line = reader.ReadLine()) != null)
					{
						if (count == 0)
						{
                            ...
                            //coverletterData.Month = DateTimeFormatInfo.CurrentInfo.MonthNames.ToList().ConvertAll(x=>x.ToLower()).IndexOf(dateinfo[1].ToLower()) + 1;
                            //coverletterData.Year = Convert.ToInt32(dateinfo[2]);
							count++;
						}
						else
						{
